...
export class ParticipantService {
  ...
  protected convertDateFromClient(participant: IParticipant): IParticipant {
    return Object.assign({}, participant, {
      dateDeNaissance: participant.dateDeNaissance?.isValid() ? participant.dateDeNaissance.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((participant: IParticipant) => {
        participant.dateDeNaissance = participant.dateDeNaissance ? dayjs(participant.dateDeNaissance) : undefined;
      });
    }
    return res;
  }

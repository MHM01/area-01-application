...
export class InscriptionService {
  ...
  protected convertDateFromClient(inscription: IInscription): IInscription {
    return Object.assign({}, inscription, {
      startDate: inscription.startDate?.isValid() ? inscription.startDate.toJSON() : undefined,
      endDate: inscription.endDate?.isValid() ? inscription.endDate.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((inscription: IInscription) => {
        inscription.startDate = inscription.startDate ? dayjs(inscription.startDate) : undefined;
        inscription.endDate = inscription.endDate ? dayjs(inscription.endDate) : undefined;
      });
    }
    return res;
  }
